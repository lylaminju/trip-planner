...
-- Commits a trip's new dates and the visit-date realignment they require in
-- one transaction, so a mid-flight failure can never strand the itinerary on
-- dates the trip no longer covers. Refuses to apply a shift computed against
-- trip dates that have since changed.
