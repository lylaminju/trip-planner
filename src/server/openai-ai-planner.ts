...
const SYSTEM_PROMPT = [
  ...
  "Visits on the same day must not overlap: each start_time must be at least the previous visit's start_time plus its duration_minutes plus realistic travel time between the two. Give a long visit its full duration rather than starting the next one partway through it.",
