...
export function formatPlaceRow(source: PlaceRowSource, options: { context?: PlaceRowContext } = {}): PlaceRowDisplay {
...
export function formatSchedule(item: ItineraryItem): string {
	if (item.visit_date && item.visit_time) return `${item.visit_date} ${item.visit_time}`;
...
function hasNestedPlace(source: PlaceRowSource): source is ItineraryItem {
	return "place" in source && typeof source.place === "object" && source.place !== null;
}
